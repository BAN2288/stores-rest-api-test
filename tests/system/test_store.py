...
class StoreTest(BaseTest):
    ...
    def test_create_duplicate_store(self):
        with self.app() as client:
            with self.app_context():
                ...
                self.assertEqual({'message': "A store with name 'test' already exists."},
    ...
    def test_store_found_with_items(self):
        with self.app() as client:
            with self.app_context():
                ...
                resp = client.get('/store/test')
                self.assertEqual(resp.status_code, 200)
                self.assertDictEqual({'name': 'test', 'items': [{'name': 'test', 'price': 1.1}]},
                                     json.loads(resp.data))
    ...
    def test_store_list_with_itmes(self):
        with self.app() as client:
            with self.app_context():
                client.post('/store/test')
                ItemModel('test', 1.1, 1).save_to_db()
                ...
                resp = client.get('/stores')
                self.assertDictEqual({'stores': [{'name': 'test', 'items': [{'name': 'test', 'price': 1.1}]}]},
                                     json.loads(resp.data))
